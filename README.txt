En este proyecto se busca documentar y llevar al formato de apunte las diapositivas del curso de econometría de series de tiempo 
del Profesor Marcelo Villena Chamorro, Facultad de Ingeniería y Ciencias, Universidad Adolfo Ibáñez, Santiago, Chile, Julio 2019.
Archivos:
	download-data.R - download csv file to a data directory/se debe trabajar
	wrangle-data.R - creates a derived dataset and saves as R object in rda directory/se debe trabajar
	analysis.R - generates a plot and saves it to the figs directory/se debe trabajar
Carpeta notas_est:
	1-"Archivo main.Tex" es el que coordina el documento. 
	2-El archivo "Archivo main.Tex" utiliza los archivos capitulo "chapxx.Tex".
	3-La edición de "main.Tex" depende del archivo "easyclass.cls"
Carpetas de respaldos:
	1-clasesTex: archivos con las diapos (en Tex) originales del Profe.
	2-codigos: codigos R utilizados en las clases.
	3-data: datasets utilizados en ejemplos/códigos de clases.
	4-referencias: referencias de las diapositivas utilizadas en clases.
Pagina de donde se saco el template para las notas:
https://www.overleaf.com/project/5d40629118b1810cb3ce9a57	